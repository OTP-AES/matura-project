...
This swapping of bytes is performed using a Rijndael S-box as the lookup table\cite{SBox}. Firstly, it is very important to note that this substitution is performed in a Galois field (also referred to as a finite field)\cite{GaloisField}, specifically $GF(2^8)$. This means that the field only contains $2^8$ elements; this makes sense, as there are only $2^8$ possible bytes.

The subbytes step can be described as 

%%The most simple method of performing this substitution is to first split the byte into two 4-bit nibbles. If the byte is represented in %%hexadecimal, then the first nibble is simply the first of the two values, the second nibble being the second of the two values.
...
a_{3,1} & a_{3,2} & a_{3,3} & a_{3,0}\end{array} \right)
\] 
